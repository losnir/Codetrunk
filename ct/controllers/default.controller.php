 ...
 *
 * @package Codetrunk
 * @filesource default.controller.php
...
class defaultController extends Controller
{
    ...
    * Shows default
    * 
    * defaultController::showDefault()
    * @return bool
    */
   function showDefault() {
      Codetrunk::getInstance()->wRenderer->appendContentHook(array($this, 'renderDefault'), func_get_args());
      return true;
   }
   ...
   function renderDefault($ctName = null, $ctCode = null, $pData = array("Submit a new Trunk", 0, false)) {
      $rememberCookie = Codetrunk::getInstance()->Controllers['trunks']->getRememberCookie();
      if($rememberCookie !== false) {
         $ctName     = ($ctName !== null ? $ctName : $rememberCookie['ctName']);
         $ctExpiry   = $rememberCookie['ctExpiry'];
      }
      ?>
               <div class="title nRound3 round3"><img src="<?=ROOT?>/images/icons/box.png" width="16" height="16" alt="" style="margin-right: 4px;" class="icon pngfix" /> <?=$pData[0]?></div>
               <form action="<?=ROOT?>/submitTrunk" method="post">
                  <input type="hidden" name="Conroller" value="submitTrunk" />
                  <input type="hidden" name="ctPKey" value="<?=$pData[1]?>" />
                  <label for="ctSyntaxLanguage" style="font: bold 15px Arial; float: left; margin-top: 2px; *margin-top: 4px;"><img src="<?=ROOT?>/images/icons/script_code.png" alt="" width="16" height="16" class="icon pngfix" />Syntax highlighting language:</label>
                     <span class="nRound3 round3" style="border: 1px solid #90a9c8; padding: 3px; background: #fff; float: left;">
                        <select id="ctSyntaxLanguage" name="ctSyntaxLanguage" style="width: 126px; padding: 0; margin: 0; border: 0;">
                  ...
                  <div class="nRound3 round3" style="margin-top: 12px; padding: 1px; border: 1px solid #90a9c8; background: #ffffff;">
                     <textarea name="ctTrunk" rows="10" cols="1" style="border: 0; width: 100%; background: #ffffff;"><?=htmlspecialchars($ctCode)?></textarea>
                  </div>
               
                  <div style="margin-top: 12px;">
                     <label for="ctName" style="font-size: 13px; margin: 4px 8px 0 0; width: 160px;" class="left">Your name is</label>
                        <input type="text" name="ctName" id="ctName" value="<?=$ctName?>" class="nRound3 nBtn" style="background: #ffffff; margin: 0; padding: 4px; width: 162px; float: left;" />
                  ...
                  <div style="margin-top: 12px;">
                     <label style="font-size: 13px; margin: 4px 8px 0 0; width: 160px; visibility: hidden;" class="left">&nbsp;</label>
                        <input type="checkbox" name="ctRemember" id="rememberMe" value="true" checked="checked" />
                           <label for="rememberMe" style="font-size: 13px; color: #42638c;"> Remember me so I'll be able to delete my trunk</label>
                  </div>
               </form>
      <?php
   }
